...
:buildPDF
    ECHO -- Building PDF --
    latexmk --enable-write18 --extra-mem-bot=100000000 -aux-directory=Debug -shell-escape -file-line-error -interaction=nonstopmode -halt-on-error -output-directory=figures -silent -xelatex %directory%/%tex%.tikz
    EXIT /B

:: Build PDF verbose
:buildPDFv
    ECHO -- Building PDF -- (v)
    latexmk --enable-write18 --extra-mem-bot=100000000 -aux-directory=Debug -shell-escape -file-line-error -interaction=nonstopmode -halt-on-error -output-directory=figures -verbose -xelatex %directory%/%tex%.tikz
    EXIT /B
