...
basis functions\footnote{Unlike the Fourier basis, the
monomial basis $\left\{1,\: x,\: x^2,\: \ldots\right\}$ used in the
Taylor series is not orthogonal, but can be orthogonalised via the
Gram-Schmidt process, yielding families such as the Legendre or
...
\subsection{Gradient}
The gradient of a scalar field is an operator \(\grad : \left( \R^n \to \R \right) \to
\mathbb{R}^n\) that maps a scalar field \(f\) to a vector field:
\begin{equation*}
...
The gradient of a scalar field \(f\left( x_1, \: \ldots, \: x_n \right)\)
in the direction of a specific basis vector can be found using the dot
product:
\begin{equation*}
...
\left( \R^n \to \R^n \right) \to \mathbb{R}\), which maps a vector field \(\symbf{F}\) to
a scalar:
\begin{equation*}
...
\end{itemize}
\subsection{Curl}
The curl of a vector field is an operator \(\curl : \left( \R^n \to \R^n \right) \to
\left( \R^n \to \R^n \right)\), which maps a vector field \(\symbf{F} : \mathbb{R}^3 \to
