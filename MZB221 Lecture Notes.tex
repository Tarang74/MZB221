...
Equivalently, the Maclaurin series expresses \(f\left( x \right)\) as a
linear combination of the \(\infty\)-dimensional monomial basis:
\begin{equation*}
    \left\{ 1,\: x,\: x^2,\: x^3,\: \ldots \right\}.
\end{equation*}
The general Taylor series uses the shifted basis:
\begin{equation*}
    \left\{ 1,\: \left( x - x_0 \right),\: {\left( x - x_0 \right)}^2,\: \ldots \right\}.
\end{equation*}
...
approximation of \(f\left( t \right)\) using a set of basis functions.
...
basis functions\footnote{Unlike the Fourier basis, the monomial basis
$\left\{1,\: x,\: x^2,\: \ldots\right\}$ used in the Taylor series is
not orthogonal, but can be orthogonalised via the Gram-Schmidt process,
yielding families such as the Legendre or Chebyshev polynomials.}:
\begin{equation*}
    \left\{ 1, \: \cos{\left( \frac{2\pi n}{T} t \right)}, \: \sin{\left( \frac{2\pi n}{T} t \right)} \right\}
\end{equation*}
for all \(n \in \mathbb{N}\). In this particular function space, the
inner product of two functions \(f\left( t \right)\) and
\(g\left( t \right)\) is defined:
\begin{equation*}
...
is one. The Fourier basis functions are \textbf{orthogonal} under this
inner product. Their inner products are given by:
\begin{align*}
...
\(f\left( t \right)\):
\begin{align*}
    f_c\left( t \right) = a_0 + \sum_{n=1}^{\infty} a_n \cos{\left( \frac{2\pi n}{T} t \right)}, \quad a_0 = \frac{2}{T} \int_{0}^{T/2} f\left( t \right) \odif{t}, \quad a_n = \frac{4}{T} \int_{0}^{T/2} f\left( t \right) \cos{\left( \frac{2\pi n}{T} t \right)} \odif{t}
\end{align*}
...
and \(a_n\):
\begin{equation*}
    a_0 = \frac{1}{T} \int_{-T/2}^{T/2} f\left( t \right) \odif{t}, \quad a_n = \frac{2}{T} \int_{-T/2}^{T/2} f\left( t \right) \cos{\left( \frac{2\pi n}{T} t \right)} \odif{t}
\end{equation*}
...
\begin{equation*}
    f_s\left( t \right) = \sum_{n=1}^{\infty} b_n \sin{\left( \frac{2\pi n}{T} t \right)}, \quad b_n = \frac{4}{T} \int_{0}^{T/2} f\left( t \right) \sin{\left( \frac{2\pi n}{T} t \right)} \odif{t}
\end{equation*}
...
\subsection{Gradient}
The gradient of a scalar field is an operator \(\grad : \left( \R^n \to
\R \right) \to \mathbb{R}^n\) that maps a scalar field \(f\) to a
vector field:
\begin{equation*}
    \grad{f} \equiv \symbf{\nabla} f = \left[ \pdv{f}{x_1}, \: \ldots, \: \pdv{f}{x_n} \right].
\end{equation*}
This operator collects all partial derivatives of \(f\) into the vector
\(\symbf{\nabla} f\).
\subsection{Directional Derivatives}
The gradient of a scalar field \(f\left( x_1, \: \ldots, \: x_n
\right)\) in the direction of a specific basis vector can be found
using the dot product:
\begin{equation*}
    \symbf{\nabla} f \cdot \symbf{e}_i = \pdv{f}{x_i}.
\end{equation*}
To find the gradient of \(f\) along an arbitrary vector \(\mathbf{u} =
\left[ u_1, \: \ldots, \: u_n \right]\), we can scale the standard basis
vectors by the components of \(\hat{\mathbf{u}}\):
\begin{equation*}
    \symbf{\nabla} f \cdot \hat{\symbf{u}}.
\end{equation*}
This is known as the \textbf{directional derivative} of \(f\) in the
direction of \(\mathbf{u}\) and is defined as follows:
\begin{equation*}
    D_{\mathbf{u}} f \equiv \pdv{f}{\symbf{u}} = \symbf{\nabla} f \cdot \hat{\symbf{u}} = \sum_{i=1}^{n} \pdv{f}{x_i} \frac{u_i}{\norm*{\symbf{u}}}.
\end{equation*}
...
\end{itemize}
To prove the first result, we will try to maximise \(D_{\symbf{u}} f\):
\begin{equation*}
    D_{\symbf{u}} f = \symbf{\nabla} f \cdot \hat{\symbf{u}} = \norm{\symbf{\nabla} f} \norm{\hat{\symbf{u}}} \cos{\theta} = \norm{\symbf{\nabla} f} \cos{\theta}.
\end{equation*}
This result is maximised when \(\theta = 0\), or when
\(\symbf{u} = \symbf{\nabla} f\). For the second result, \(\theta = \pi\),
so that \(\symbf{u} = -\symbf{\nabla} f\).
...
The divergence of a vector field is an operator \(\divergence : \left(
\R^n \to \R^n \right) \to \mathbb{R}\), which maps a vector field
\(\symbf{F}\) to a scalar:
\begin{equation*}
...
\end{itemize}
\subsection{Curl}
The curl of a vector field is an operator \(\curl : \left( \R^n \to
\R^n \right) \to \left( \R^n \to \R^n \right)\), which maps a vector
field \(\symbf{F} : \mathbb{R}^3 \to \mathbb{R}^3\) to another vector
field \(\symbf{G} : \mathbb{R}^3 \to \mathbb{R}^3\):
\begin{equation*}
    \curl \symbf{F} = \symbf{\nabla} \times \symbf{F} =
    \begin{vmatrix}
...
\begin{itemize}
    ...
    \item In the \(x\)-direction, the region is bounded by the curves:
          \begin{alignat*}{2}
              ...
              c                 & \hspace{0.5em} \leqslant \hspace{0.5em} y &  & \hspace{0.5em} \leqslant \hspace{0.5em} d
          \end{alignat*}
...
along a curve parametrised by the path \(\symbf{r}\left( t \right)\).
Here we recall the unit tangent vector at each point is
\begin{equation*}
    \symbf{\hat{\tau}}\left( t \right) = \frac{\symbf{r}'\left( t \right)}{\norm*{\symbf{r}'\left( t \right)}},
\end{equation*}
so the tangential component of \(\symbf{F}\) is \(\symbf{F} \cdot \symbf{\hat{\tau}}\).
This shows the vector line integral is precisely a scalar line integral of
that tangential component:
\begin{align*}
    \int_{\mathscr{C}} \symbf{F} \cdot \odif{\symbf{r}}
     & = \int_{\mathscr{C}} \left( \symbf{F} \cdot \symbf{\hat{\tau}} \right) \odif{r}                                                                                                                     \\
     & = \int_a^b \left( \symbf{F}\left( \symbf{r}\left( t \right) \right) \cdot \frac{\symbf{r}'\left( t \right)}{\norm*{\symbf{r}'\left( t \right)}} \right) \norm*{\symbf{r}'\left( t \right)} \odif{t} \\
     & = \int_a^b \symbf{F}\left( \symbf{r}\left( t \right) \right) \cdot \symbf{r}'\left( t \right) \odif{t}
\end{align*}
Here the differential element \(\odif{\symbf{r}}\) takes the form:
\begin{equation*}
    \odif{\symbf{r}} = \odv{\symbf{r}}{t} \odif{t} = \symbf{r}'\left( t \right) \odif{t}
\end{equation*}
where \(\symbf{r}'\left( t \right)\) is the velocity of the path. It can
be related to the scalar differential element \(\odif{r}\) as follows:
\begin{equation*}
    \odif{\symbf{r}} = \symbf{r}'\left( t \right) \odif{t} = \frac{\symbf{r}'\left( t \right)}{\norm*{\symbf{r}'\left( t \right)}} \norm*{\symbf{r}'\left( t \right)} \odif{t} = \symbf{\hat{\tau}}\left( t \right) \odif{r}.
\end{equation*}
...
\end{equation*}
where \(\norm*{\symbf{r}_s \times \symbf{r}_t}\) is the area of
the parallelogram spanned by \(\symbf{r}_s\) and \(\symbf{r}_t\). This
scalar quantity represents the scale factor between the area element
\(\odif{\sigma}\) on the surface and the projected area element
\(\odif{s} \odif{t}\) in the parameter space. Therefore, the surface
integral of \(f\) over the surface \(\mathscr{S}\) is given by:
\begin{equation*}
    \iint_{\mathscr{S}} f \odif{\sigma} = \iint_T f\left( \symbf{r}\left( s,\: t \right) \right) \norm*{\symbf{r}_s \times \symbf{r}_t} \odif{s} \odif{t},
\end{equation*}
where \(T\) is the region in the projected \(s\)-\(t\) space.
\subsubsection{Surface Area}
The surface area of the surface \(\mathscr{S}\) is defined as the
surface integral of a uniform field:
\begin{equation*}
    A = \iint_{\mathscr{S}} \odif{\sigma} = \iint_T \norm*{\symbf{r}_s \times \symbf{r}_t} \odif{s} \odif{t}.
\end{equation*}
...
integral over \(f\) to:
\begin{align*}
    \iint_{\mathscr{S}} f \odif{\sigma} & = \iint_{T} f\left( \symbf{r}\left( x,\: y \right) \right) \norm*{\symbf{r}_x \times \symbf{r}_y} \odif{x} \odif{y} \\
                                        & = \iint_{T} f\left( x,\: y,\: g\left( x,\: y \right) \right) \norm*{
                                                                                                           \begin{bmatrix}
                                                                                                               1 \\
                                                                                                               0 \\
                                                                                                               g_x
                                        ...
                                        & = \iint_{T} f\left( x,\: y,\: g\left( x,\: y \right) \right) \sqrt{g_x^2 + g_y^2 + 1} \odif{x} \odif{y}.
\end{align*}
\subsection{Surface Integrals of Vector Fields}
Surface integrals of vector fields take the form:
\begin{equation*}
...
Here we recall the unit normal vector at each point is
\begin{equation*}
    \hat{\symbf{n}}\left( s,\: t \right) = \frac{\symbf{r}_s \times \symbf{r}_t}{\norm*{\symbf{r}_s \times \symbf{r}_t}},
\end{equation*}
so the normal component of \(\symbf{F}\) is \(\symbf{F} \cdot \hat{\symbf{n}}\).
This shows the vector surface integral is precisely a scalar surface
integral of that normal component:
\begin{align*}
    \iint_{\mathscr{S}} \symbf{F} \cdot \odif{\symbf{\sigma}}
     & = \iint_{\mathscr{S}} \left( \symbf{F} \cdot \hat{\symbf{n}} \right) \odif{\sigma}                                                                                                                                            \\
     & = \iint_T \left( \symbf{F}\left( \symbf{r}\left( s,\: t \right) \right) \cdot \frac{\symbf{r}_s \times \symbf{r}_t}{\norm*{\symbf{r}_s \times \symbf{r}_t} } \right) \norm*{\symbf{r}_s \times \symbf{r}_t} \odif{s} \odif{t} \\
     & = \iint_T \symbf{F}\left( \symbf{r}\left( s,\: t \right) \right) \cdot \left( \symbf{r}_s \times \symbf{r}_t \right) \odif{s} \odif{t}.
\end{align*}
Here the differential element \(\odif{\symbf{\sigma}}\) is a product of
the unit normal vector and the scalar differential element:
\begin{equation*}
    \odif{\symbf{\sigma}} = \hat{\symbf{n}} \odif{\sigma} = \left( \symbf{r}_s \times \symbf{r}_t \right) \odif{s} \odif{t}
\end{equation*}
where \(\symbf{r}_s \times \symbf{r}_t\) is some normal vector to the
surface \(\mathscr{S}\).
Note that this integral represents the outward or inward flow of
\(\symbf{F}\) through \(\mathscr{S}\), where the direction of flow
depends on the sign of \(\symbf{\hat{n}}\).
\subsubsection{Explicit Surfaces}
When the surface \(\mathscr{S}\) can be expressed explicitly by the
function
\begin{equation*}
...
\begin{align*}
    \Phi = \iint_{\mathscr{S}} \symbf{F} \cdot \odif{\symbf{\sigma}} = \iint_{\mathscr{S}} \left( \symbf{F} \cdot \hat{\symbf{n}} \right) \odif{\sigma} & = \iint_{T} \symbf{F}\left( \symbf{r}\left( x,\: y \right) \right) \cdot \left( \symbf{r}_x \times \symbf{r}_y \right) \odif{x} \odif{y} \\
                                                                                                                                                        & = \iint_{T} \symbf{F}\left( x,\: y,\: g\left( x,\: y \right) \right) \cdot \left(
    \begin{bmatrix}
        1 \\
        0 \\
        g_x
    ...
    \end{bmatrix}
    \right) \odif{x} \odif{y}                                                                                                                                                                                                                                                                                \\
                                                                                                                                                        & = \iint_{T} \symbf{F}\left( x,\: y,\: g\left( x,\: y \right) \right) \cdot
    \begin{bmatrix}
        -g_x \\
        -g_y \\
        1
    \end{bmatrix}
    \odif{x} \odif{y}                                                                                                                                                                                                                                                                                        \\
                                                                                                                                                        & = \iint_{T} \left( - F_1 g_x - F_2 g_y + F_3 \right) \odif{x} \odif{y}.
\end{align*}
...
This expands to
\begin{equation*}
    \iint_R \left( \pdv{F_2}{x} - \pdv{F_1}{y} \right) \odif{x} \odif{y} = \oint_{\partial R} F_1 \odif{x} + F_2 \odif{y}.
\end{equation*}
...
\begin{enumerate}
    ...
    \item For every irreducible quadratic factor \(\left( a s^2 + bs +
          c \right)^m\) of \(Q\left( s \right)\), we must include terms
          of the form:
          \begin{equation*}
